...
class MultimodalBertEncoder(nn.Module):
    ...
    def forward(self, input_txt_left,input_txt_right, attention_mask_left, attention_mask_right,segment_left,segment_right, input_img_left,input_img_right):
        ...
        return self.pooler(encoded_layers_left[-1]),self.pooler(encoded_layers_right[-1])
...
class MultimodalBertClf(nn.Module):
    ...
    def forward(self, txt_left,txt_right, mask_left,mask_right, segment_left,segment_right, img_left,img_right):
        x,y = self.enc(txt_left,txt_right, mask_left,mask_right, segment_left,segment_right, img_left,img_right)
        # xx = x.unsqueeze(1)
        # yy = y.unsqueeze(1)
        # z = self.sim(xx, yy)
        return x,y
